...
const BarChart = () => {
  ...
  for (let i = 1; i <= 10; i++) {
    let district = "Quận " + i;
    let notProcessed = ticket.filter(
      (ticket) =>
        ticket.status === "not processed" && ticket.district === "Quận " + i
    ).length;
    let inProgress = ticket.filter(
      (ticket) =>
        ticket.status === "in progress" && ticket.district === "Quận " + i
    ).length;
    let done = ticket.filter(
      (ticket) => ticket.status === "done" && ticket.district === "Quận " + i
    ).length;
    let detailData = {
      district: district,
