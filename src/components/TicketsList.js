...
const TicketsList = () => {
  ...
  const handleFilter = (e) => {
    e.preventDefault();
    const x = ticketList.filter((t) => 
    (filter.status.length>0 ? filter.status.includes(t.status) : true ) && 
    (filter.priority.length>0 ? filter.priority.includes(t.priority) : true) && 
 ...
 <Row style={{border:"1px solid gray", marginTop:"3%", marginBottom:"3%", borderRadius:"5px"}}>
   <Col>
   <Select
    mode="multiple"
    showArrow
...
<Select
  ...
  />
    ...
    <Col>
    <Select mode="tags" style={{ width: '90%', margin:"3%" }} placeholder="Choose area to search" onChange={handleChange}>
    {children}
  </Select>
    </Col>
    <Button type="primary" style={{width:"10%", margin:"0.8%"}} onClick={handleFilter}>OK</Button>
</Row>
